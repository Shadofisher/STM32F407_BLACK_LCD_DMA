...
void __LCD_Fill(uint16_t sx,uint16_t sy,uint16_t ex,uint16_t ey,uint16_t color)
{
	uint16_t i,j;
	uint16_t xlen=0;
	xlen=ex-sx+1;
...
static void tft_flush_cb(lv_disp_drv_t * drv, const lv_area_t * area, lv_color_t * color_p)
{
	...
	LCD_WR_REG(0x2A);
	LCD_WR_DATA((uint8_t)((col_start) >> 8));
	LCD_WR_DATA((uint8_t)((col_start) >> 0));
	LCD_WR_DATA((uint8_t)((col_end) >> 8));
	...
	LCD_WriteRAM_Prepare();     			//¿ªÊŒÐŽÈëGRAM
	if(HAL_DMA_Start_IT(&hdma_memtomem_dma2_stream0, (uint32_t)color_p, (uint32_t)0x60080000, xlen) != HAL_OK)
	{
		/* Transfer Error */
		Error_Handler();
	}
